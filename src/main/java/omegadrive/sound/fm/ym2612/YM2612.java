 ...
 * Resuscitated for perf reasons, should not be using it...
 * @deprecated only use for performance reason, much faster than Nuke2612
 */
public final class YM2612 extends Ym2612RegSupport implements MdFmProvider {

    private final static Logger LOG = LogHelper.getLogger(YM2612.class.getSimpleName());
...
//                Long.toHexString(YM2612_TimerBcnt));
    ...
    private void logEvent(String str, int addr, int data) {
        ...
        String s = LogHelper.formatMessage("{}, {}, {}, {}", delta, str, Integer.toHexString(addr), Integer.toHexString(data));
        LOG.info(s);
    ...
    private final void synchronizeTimers(int length) {
        ...
        if ((YM2612_Mode & FM_MODE_LOAD_A_MASK) != 0) {   //TimerA ON
            ...
            if (YM2612_TimerAcnt <= 0) {
                ...
                if (verbose) LOG.info("Timer A expired at: {}, " +
        ...
        if ((YM2612_Mode & FM_MODE_LOAD_B_MASK) > 0) { //TimerB ON
            ...
            if (YM2612_TimerBcnt <= 0) {
                ...
                if (verbose) LOG.info("Timer B expired at: {}, " +
    ...
    private final int setYM(int address, int data) {       // INT, UCHAR
        ...
        switch (address) {
            ...
            case 0x24: //Timer A MSB
                ...
                if (verbose) LOG.info("Set Timer A MSB: {}, count: {}", YM2612_TimerAL,
            ...
            case 0x25: //Timer A LSB
                ...
                YM2612_TimerAL = getTimerACount(YM2612_TimerA);
                if (verbose) LOG.info("Set Timer A LSB: {}, count: {}", YM2612_TimerAL,
                        YM2612_TimerAcnt, verbose);
                break;
            case 0x26:
                YM2612_TimerB = data;
                YM2612_TimerBL = getTimerBCount(YM2612_TimerB);
                if (verbose) LOG.info("Set Timer B: {}, count: {}", YM2612_TimerBL,
            ...
            case 0x2B:
                ...
                break;
            default:
                if (!isResetting) LOG.warn("Unexpected write to addr: {}, data: {}", th(address), th(data));
        }
        return 0;
    }
