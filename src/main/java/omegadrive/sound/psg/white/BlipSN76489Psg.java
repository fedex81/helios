...
public class BlipSN76489Psg extends SN76489Psg {

    private BlipSoundProvider blipPcmProvider;
    ...
    public static BlipSN76489Psg createInstance(RegionDetector.Region region, int outputSampleRate) {
        ...
        s.blipPcmProvider = new BlipSoundProvider("psg", RegionDetector.Region.EUROPE, AbstractSoundManager.audioFormat,
                DEFAULT_CLOCK_RATE);
        return s;
    }
    ...
    public void updateRate(RegionDetector.Region region, int clockRate) {
        blipPcmProvider.updateRegion(region, clockRate);
    }
    ...
    public void tick() {
        ...
        blipPcmProvider.playSample(output[0] << 8, output[0] << 8);
    }

    @Override
    public SampleBufferContext getFrameData() {
        onNewFrame();
        return blipPcmProvider.getDataBuffer();
    }
