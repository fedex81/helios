 ...
 * Copyright 2024
 */
public class CdcImplOld implements Cdc {

    private final static Logger LOG = LogHelper.getLogger(CdcImplOld.class.getSimpleName());
    ...
    public CdcImplOld(MegaCdMemoryContext mc, McdSubInterruptHandler ih) {
        memoryContext = mc;
        interruptHandler = ih;
    ...
    private int controllerRead() {
        int data = 0;
        if (cdcContext.address > CdcModel.CdcAddressRead.STAT3.ordinal()) {
            increaseCdcAddress();
            return 0xFF;
        }
        CdcModel.CdcAddressRead addressRead = cdcAddrReadVals[cdcContext.address];
        switch (addressRead) {
            //COMIN: command input
            case COMIN -> {
...
//                assert false; //TODO mcd-ver reads all the regs
    ...
    private void controllerWrite(byte data) {
        if (cdcContext.address > CdcModel.CdcAddressWrite.RESET.ordinal()) {
            increaseCdcAddress();
            return;
        }
        CdcModel.CdcAddressWrite addressWrite = cdcAddrWriteVals[cdcContext.address];
...
//        poll();
        ...
        if (cdcContext.control.writeRequest > 0) {
            ...
            if (sector < 0) {
                LOG.info("Ignore decoding track {}, sector: {}({})",
                        track01.trackData.getNumber(), sector, msfHolder);
                return;
            }
