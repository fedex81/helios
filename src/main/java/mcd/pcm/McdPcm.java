 ...
 * Copyright 2024
 */
public interface McdPcm extends BufferUtil.StepDevice, SystemProvider.NewFrameListener {
    ...
    int PCM_REG_MASK = PCM_REG_SIZE - 1;

    int PCM_START_RAMPTR_REGS = 0x20;
    int PCM_END_RAMPTR_REGS = 0x40;
    int PCM_LOOP_MARKER = 0xFF;

    int PCM_START_WAVE_DATA_WINDOW = 0x1000;
    int PCM_END_WAVE_DATA_WINDOW = 0x2000;

    int PCM_WAVE_DATA_WINDOW_MASK = PCM_END_WAVE_DATA_WINDOW - PCM_START_WAVE_DATA_WINDOW - 1;

    //external access doubles the address, adjust the mask accordingly
    int PCM_WAVE_DATA_WINDOW_MASK_EXT = PCM_END_WAVE_DATA_WINDOW - 1;
    int PCM_WAVE_DATA_SIZE = 0x10000; //64Kb
    int PCM_ADDRESS_MASK = PCM_WAVE_DATA_SIZE - 1;
    ...
    int PCM_FIX_POINT_DECIMAL = 11;


    int PCM_WAVE_BANK_SHIFT = 12;


    int MCD_PCM_DIVIDER = 384;
    double pcmSampleRateHz = MCD_SUB_68K_CLOCK_MHZ / MCD_PCM_DIVIDER;

    int PCM_SAMPLE_RATE_HZ = (int) pcmSampleRateHz;

    void pcmDataWriteByte(int addr, int value);
    ...
    class McdPcmImpl implements McdPcm {

        private final ByteBuffer waveData;
        ...
        private McdPcmImpl(boolean enableSound) {
...
//            logAccessReg(getPcmReg(address), SUB_M68K, address, size, true);
        ...
        private void generateOneSample() {
            ...
            for (PcmChannelContext channel : chan) {
				...
				*/
                    ...
                    // Shifts the 8th bit to the 1st position (0 or 1), multiplies by 2, and subtracts 1
                    int sign = ((pcm & 0x80) >> 6) - 1;
                    int ampl = pcm & 0x7F;
                    ls += sign * ((ampl * channel.factorl) >>> 5);
                    rs += sign * ((ampl * channel.factorr) >>> 5);
                }
            }
            ...
            ls = clampToShort(ls);
            if (verbose && prev != ls)
                LOG.info("PCM left clamp {} -> {}", th(prev), th(ls));

            prev = rs;
            rs = clampToShort(rs);
            if (verbose && prev != rs)
                LOG.info("PCM right clamp {} -> {}", th(prev), th(rs));
