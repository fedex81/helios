...
//                cddCounter    = 0; //TODO
                ...
                if (verbose) LOG.info(cddCommand + " to sector: {}({})", msfHolder, th(lba));
            }
    ...
    private void handleRequestCommand() {
        ...
        switch (request) {
            case AbsoluteTime -> {
                ...
                int status8 = (isAudio ? 1 : 0) << 2; //TODO should be flipped? US bios seems to imply isAudio = 1
...
//                auto [minute, second, frame] = CD::MSF(session.tracks[track].indices[1].lba);
    ...
    private void setIoStatus(CddStatus status) {
        if (status != cddContext.io.status) {
            ...
            //updateStatus(0, status.ordinal()); //TODO this breaks BIOS
        }
    }
