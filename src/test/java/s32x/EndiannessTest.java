...
public class EndiannessTest {
    ...
    public void testLong() {
        ...
        b = reset(numBytes);
        valL = 0x11223344;
        BufferUtil.writeBufferRaw(b, 0, valL, Size.LONG);
        actL = BufferUtil.readBuffer(b, 0, Size.LONG);
        Assertions.assertEquals(valL, actL);

        actW = BufferUtil.readBuffer(b, 0, Size.WORD);
        valW = valL >> 16;
        Assertions.assertEquals(valW, actW);

        actW = BufferUtil.readBuffer(b, 2, Size.WORD);
        valW = valL & 0xFFFF;
        Assertions.assertEquals(valW, actW);

        actB = (byte) BufferUtil.readBuffer(b, 0, Size.BYTE);
        Assertions.assertEquals(valL >> 24, actB);

        actB = (byte) BufferUtil.readBuffer(b, 1, Size.BYTE);
        Assertions.assertEquals((valL >> 16) & 0xFF, actB);

        actB = (byte) BufferUtil.readBuffer(b, 2, Size.BYTE);
        Assertions.assertEquals((valL >> 8) & 0xFF, actB);

        actB = (byte) BufferUtil.readBuffer(b, 3, Size.BYTE);
    ...
    public void testByte() {
        ...
        valB = 0x11;
        BufferUtil.writeBufferRaw(b, 0, valB, Size.BYTE);
        actB = (byte) BufferUtil.readBuffer(b, 0, Size.BYTE);
        ...
        b = ByteBuffer.allocate(numByte);
        valB = 0x22;
        BufferUtil.writeBufferRaw(b, 1, valB, Size.BYTE);
        actB = (byte) BufferUtil.readBuffer(b, 1, Size.BYTE);
        Assertions.assertEquals(valB, actB);
    ...
    public void testWord() {
        ...
        b = ByteBuffer.allocate(numByte);
        valW = 0x1122;
        BufferUtil.writeBufferRaw(b, 0, valW, Size.WORD);
        actW = BufferUtil.readBuffer(b, 0, Size.WORD);
        Assertions.assertEquals(valW, actW);
